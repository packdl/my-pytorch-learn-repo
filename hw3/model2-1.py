...
model = AutoModelForQuestionAnswering.from_pretrained("albert/albert-base-v2").to(device)
...
if __name__=='__main__': 
    ...
    #scheduler = transformers.get_linear_schedule_with_warmup(optimizer, 1000, len(train_dl) * (epochs+1))
    #scheduler = transformers.get_polynomial_decay_schedule_with_warmup(optimizer, 0, len(train_dl) * epochs, lr_end = 5e-06)
    scheduler = transformers.get_polynomial_decay_schedule_with_warmup(optimizer, 0, len(train_dl) * epochs, lr_end = 5e-06)
    ...
    torch.save(best_model['params'], 'albert_finetuned2-1.pth')
