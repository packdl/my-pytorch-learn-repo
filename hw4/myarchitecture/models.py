...
class NetD(nn.Module):
    def __init__(self):
        ...
        self.sequence = nn.Sequential(
            ...
            nn.BatchNorm2d(feat2 * 2, momentum=.5),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. ``(ndf*2) x 16 x 16``
            nn.Conv2d(feat2 * 2, feat2 * 4, 4, 2, 1, bias=False),
            nn.BatchNorm2d(feat2 * 4, momentum =.5),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. ``(ndf*4) x 8 x 8``
            nn.Conv2d(feat2 * 4, feat2 * 8, 4, 2, 1, bias=False),
            nn.BatchNorm2d(feat2 * 8, momentum=.5),
            nn.LeakyReLU(0.2, inplace=True),
            #nn.Conv2d(feat2 * 8, 1, 2, 1, 0, bias=False),
            #nn.Sigmoid()
        )
        self.last_conv2d = nn.Conv2d(feat2 * 8*2, 1, 2, 1, 0, bias=False)
...
if __name__ == '__main__':
    ...
    dis = NetD()
    out2 = dis(torch.rand(5,3,32,32), torch.randint(0,10,(5,))).squeeze()
    print(out2.shape)
    print(out2)
    #print(dis)
