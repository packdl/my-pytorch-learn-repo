...
class NetD(nn.Module):
    def __init__(self):
        ...
        self.label_class = nn.Linear(4*4* feat2 * 8, 10)
    ...
    def forward(self, inputs, label):
        ...
        label_prob = self.softmax(self.label_class(output.view(-1, 4*4*feat2*8)))
        
        output = self.last_conv2d(output)
        data  = self.flatten(output)
        # print(f'{output.shape = }')
        
        
        real_or_fake = self.sigmoid(data)
        return real_or_fake, label_prob
...
if __name__ == '__main__':
    ...
    dis = NetD()
    real_or_fake, label_prob = dis(torch.rand(5,3,64,64), torch.randint(0,10,(5,)))
    print(label_prob.shape)
    print(real_or_fake.shape)
